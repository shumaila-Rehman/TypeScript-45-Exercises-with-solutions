...
// Make a list of five or more usernames called current_users.
let current_user = ["Uabaid", "Nida","Ali", "Nimra"];


// Make another list of five usernames called new_users. Make sure one or two of the 
// new usernames are also in the current_users list.  
let new_user =["warda", "aLi", "Nida", " Amara"];
...
//  is main for each ka method lagate hain. aur some ka method use karte .. aur sath hi  lower case 
// main convert kar k condition b laga dyte hain..

new_user.forEach(one =>
    {
        let our_conditions =current_user.some(current_one => current_one.toLowerCase() === one.toLowerCase())
        if(our_conditions)
            {
                console.log(`sorry ${one}, is already taken! `)

        }else {
            console.log(`user ${one} is available ! `)
